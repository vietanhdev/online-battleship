 /**
 *  JeelizAR - https://github.com/Jeeliz/JeelizAR
 *
 * Copyright 2018  ( https://Jeeliz.com )
 *
...
"use strict;"
var MediaStreamAPIHelper={
  get_videoElement: function() {
    if (!MediaStreamAPIHelper.compat()) return false;
  ...
  check_isApple: function(){
    return MediaStreamAPIHelper.check_isSafari() || MediaStreamAPIHelper.check_isIOS();
  },
...
compat: function() {
  ...
  create_fallbackConstraints: function(constraints){
    ...
    const add_constraintsTryon=function(constraintConstructor){
      var newConstraints=MediaStreamAPIHelper.clone_constraints(constraints);
    ...
    if (constraints['video']['width'] && constraints['video']['height']){
      ...
      add_constraintsTryon(function(newConstraints){
        return MediaStreamAPIHelper.switch_widthHeight(newConstraints);
      });
    }
    ...
    if (constraints['video']['facingMode']){
      ...
      if (constraints['video']['width'] && constraints['video']['height']){
        add_constraintsTryon(function(newConstraints){
          MediaStreamAPIHelper.switch_widthHeight(newConstraints);
          delete(newConstraints['video']['facingMode']);
          return newConstraints;
        });
      }
  ...
  switch_whIfPortrait: function(mandatory){
    if (MediaStreamAPIHelper.is_portrait()){ //portrait display
  ...
  mute: function(video){
    video['volume']=0.0;
    MediaStreamAPIHelper.enable_HTMLprop(video, 'muted');
    
    if (MediaStreamAPIHelper.check_isSafari()){ //MANTIS 182 
            console.log('INFO in MediaStreamAPIHelper - mute() : Safari detected');
            if (video['volume']===1){
                console.log('WARNING in MediaStreamAPIHelper - mute() : cannot mute the video. F****ing Safari !');
                function doMute(){
          video['volume']=0.0;
                    console.log('INFO in MediaStreamAPIHelper - mute() : mute this fucking volume by a fucking user action.');
  ...
  get_raw: function(video, callbackSuccess, callbackError, constraints){
    ...
    const on_successRaw=function(video, localMediaStream, optionsReturned){
      if (isErrorRaw){
                console.log('WARNING in MediaStreamAPIHelper - get_raw(): cannot launch callbackSuccess because an error was thrown');
        console.log(JSON.stringify(constraints.video));
                return;
      }
            console.log('INFO in MediaStreamAPIHelper - get_raw(): callbackSuccess called with constraints=');
      console.log(JSON.stringify(constraints.video));
    ...
    const promise=navigator['mediaDevices'].getUserMedia(constraints).then(
      ...
      function(localMediaStream) {
                console.log('INFO in MediaStreamAPIHelper - get_raw(): videoStream got');
              ...
              try {
                ...
                if (mediaStreamTrack){
                  ...
                  optionsReturned.settings = mediaStreamTrack['getSettings']();
                                ...
                                console.log('WARNING in MediaStreamAPIHelper - get_raw(): Image Capture API not found');
                              }

              if (MediaStreamAPIHelper.check_isApple()){
              	              	console.log('WARNING in MediaStreamAPIHelper - Apple device detected, add the video element to the DOM.');
                ...
                if (!video['parentNode'] || video['parentNode']===null){
                  ...
                  MediaStreamAPIHelper.mute(video);
                  ...
                  setTimeout(function(){
                    ...
                    //from https://github.com//FaceFilter/issues/45
                    setTimeout(function () {
                     video['play']();
                   }, 100);
        ...
        function onMetaDataLoaded() {
                    console.log('INFO in MediaStreamAPIHelper - get_raw() : video.onloadedmetadata dispatched');
          ...
          if (typeof(playPromise) === 'undefined') {
            ...
            playPromise.then(function() {
                        console.log('INFO in MediaStreamAPIHelper - get_raw() : playPromise accepted');
                        callSuccessIfPlaying();
          }).catch(function(error) {
                        console.log('ERROR in MediaStreamAPIHelper - get_raw() : playPromise failed - ', error);
        ...
        if (typeof(video.srcObject)!=='undefined'){
          ...
          video['videoStream']=localMediaStream;
                    console.log('WARNING in MediaStreamAPIHelper - get_raw() : video.srcObject is not implemented. Old browser ?');
                  }
   ...
   */
   ...
   get: function(video, callbackSuccess, callbackError, mandatory) {
    if (!video){
            console.log('ERROR in MediaStreamAPIHelper.js - get() : No video provided');
            if (callbackError){
    ...
    if (!MediaStreamAPIHelper.compat()){
            console.log('ERROR in MediaStreamAPIHelper.js - get() : No getUserMedia API found !');
            if (callbackError){
    ...
    if (mandatory && mandatory['video']){
      ...
      if (MediaStreamAPIHelper.check_isIOS()){ //otherwise video freeze quickly
                console.log('INFO in MediaStreamAPIHelper() - get() : iOS detected');
        ...
        const iOSVersion = MediaStreamAPIHelper.get_IOSVersion();
        //IOS is <12.2:
        if (iOSVersion[0]<12
          || (iOSVersion[0]===12 && iOSVersion[1]<2)){
          MediaStreamAPIHelper.switch_whIfPortrait(mandatory);
      }

      } /*else if (MediaStreamAPIHelper.check_isMobile()){
                console.log('INFO in MediaStreamAPIHelper() - get() : isMobile but not iOS');
              } //end if mobile
      */
    ...
    MediaStreamAPIHelper.enable_HTMLprop(video, 'autoplay');
    MediaStreamAPIHelper.enable_HTMLprop(video, 'playsinline');
    ...
    if (isAudio){
      ...
      MediaStreamAPIHelper.enable_HTMLprop(video, 'muted');
        ...
        console.log('INFO in MediaStreamAPIHelper() - get() : constraints =', JSON.stringify(mandatory));  
      

    MediaStreamAPIHelper.get_raw(video, callbackSuccess, function(err){
            console.log('WARNING in MediaStreamAPIHelper() - get() : cannot request video with this constraints. Downgrade constraints. err=', err);
            var fallbackConstraints=MediaStreamAPIHelper.create_fallbackConstraints(mandatory);
  ...
  }, //end MediaStreamAPIHelper.get()
  
  get_videoDevices: function(callback){
    if (!navigator['mediaDevices'] || !navigator['mediaDevices']['enumerateDevices']) {
            console.log("INFO in MediaStreamAPIHelper - get_videoDevices() : enumerateDevices() not supported");
    ...
    .then(function(devices) {
      ...
      callback(videoDevices, false);
            ...
            console.log('ERROR in MediaStreamAPIHelper - get_videoDevices() : no devices founds');
            callback(false, 'NODEVICESFOUND');
    }
  })
    .catch(function(err) {
            console.log('ERROR in MediaStreamAPIHelper - get_videoDevices() : enumerateDevices promise rejected', err);
            callback(false, 'PROMISEREJECTED');
    });
  }, //end get_videoDevices()

  //use image capture api
  //test it with: MediaStreamAPIHelper.change_setting(debugVideoOptions.mediaStreamTrack, 'exposureCompensation', 3000)
  change_setting: function(mediaStreamTrack, settingId, settingValue){
    ...
    .catch(function(errMsg){
            console.log('ERROR in MediaStreamAPIHelper - change_setting(): ', errMsg);
          });
  }
};
