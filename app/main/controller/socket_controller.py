...
from ..service.game_service import send_command, get_a_room
from ..service.socket_service import save_user_with_session, delete_session, get_user_id_by_sid
...
def registerSessionId(request_object):
    ...
    if user:
        ...
        save_user_with_session(session_id=request.sid, user_id=user.id, remain_sec=remain_sec)
...
def newCommand(request_object):
    ...
    if not user:
        ...
        if room is None:
        ...
        elif room.check_exist_player(user) == False:
            response_object = {
                'status': 'fail',
            ...
            response_command, receive_event = send_command(user, room, command)
            if response_command is not None:
                emit('response_command', response_command, broadcast=False, namespace='/rooms')
